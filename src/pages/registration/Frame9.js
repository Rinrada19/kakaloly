...
function WeightGoalInput({ nextStep, prevStep }) {
  const [user] = useState(users[0]); // ดึงข้อมูลผู้ใช้คนแรก
  const [goalWeight, setGoalWeight] = useState(''); // น้ำหนักที่ต้องการลด
  ...
  const handleInputChange = (e) => {
    ...
    const maxWeightLoss = user.wight; // ใช้น้ำหนักปัจจุบันจาก MockData

    if (value > maxWeightLoss) {
      setError(`กรุณากรอกตัวเลขไม่เกินน้ำหนักปัจจุบัน (${maxWeightLoss} กิโลกรัม)`);
    } else if (value <= 0) {
